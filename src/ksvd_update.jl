...
Notes: `merge_all_timers` has no effect here and is only provided for consistency.
"""
function ksvd_update(method::LegacyKSVD, Y::AbstractMatrix, D::AbstractMatrix, X::AbstractMatrix; timer::Union{TimerOutput, Nothing}=nothing, merge_all_timers=false)
...
function ksvd_update(method::Union{ParallelKSVD{false}, BatchedParallelKSVD{false}}, Y::AbstractMatrix{T}, D::AbstractMatrix{T}, X::AbstractMatrix{T};
    ...
    @timeit_debug timer "ksvd_update" begin
        ...
        N = size(Y, 2)
        @timeit_debug timer "Copy and precompute buffers" begin
            X_cpy = copy(X)
        ...
        @inbounds for index_batch in index_batches
            ...
            @timeit_debug timer "Update errors" begin; end
        ...
        @inbounds for index_batch in index_batches
            ...
            tforeach(cenumerate(index_batch); scheduler) do (buf_idx, k)
                ...
                @timeit_debug to_ "compute E_Ω" begin
                    ...
                    @timeit_debug to_ "compute matrix vector product" begin; end
                    @timeit_debug to_ "compute outer product" begin
